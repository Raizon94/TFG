# 02 — Tests del Motor ML (SVD + Fold-in)

> **Objetivo**: Verificar la lógica del motor de ML a nivel unitario: carga de modelos, inferencia fold-in, ventana deslizante, reentrenamiento.
>
> **Requisito**: MySQL corriendo con `tfg_bd`, modelos entrenados en `backend/app/models/`
...
def test_training_metadata(tr: TestResult):
    ...
    has_fields = all(k in meta for k in ["timestamp", "order_count"])
    passed = has_fields and meta["order_count"] > 0
    tr.add("Training metadata",
           passed,
           f"fecha={meta.get('timestamp', 'N/A')}, "
           f"pedidos={meta.get('order_count', 'N/A')}")
...
def test_catalog_coverage(tr: TestResult, svd_artifact: dict):
    ...
    passed = coverage > 20  # Al menos 20% del catálogo cubierto
...
| Varianza explicada SVD | 50-60% |
| Latencia fold-in | < 5ms |
| Cobertura del catálogo | > 20% |
| Diversidad | > 0.3 |
| Ventana deslizante | MAX_WINDOW=8 activos |
